...
@push('js')
    ...
    <script>
        window.data_url = "{!! route('admin.get.trashed.stories') !!}";
        window.columns = [
            {data: "id"},
            {data: "story_image"},
            {data: "mawhob_id"},
    ...
    <script type="text/javascript">
        ...
        ///////////////////////////////////////////////////
        /// force Destroy Story
        $(document).on('click', '.force_delete_story_btn', function (e) {
            e.preventDefault();
            var id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    // Delete User
                    $.ajax({
                        url: '{!! route('admin.force.destroy.story') !!}',
                        data: {id, id},
                        type: 'post',
                        dataType: 'json',
        ...
        ////////////////////////////////////////////////////
        // restore  Story
        $(document).on('click', '.restore_story_btn', function (e) {
            e.preventDefault();
            var id = $(this).data('id');

            $.ajax({
                url: "{{route('admin.restore.story')}}",
                data: {id, id},
                type: 'post',
                dataType: 'JSON',
                beforeSend: function () {
                ...
                success: function (data) {
                    ...
                    if (data.status == true) {
                        Swal.fire({
                            ...
                            icon: "success",
                            allowOutsideClick: false,
                            customClass: {confirmButton: 'restore_story_button'}
                        });
                        $('.restore_story_button').click(function () {
                            updateDataTable();
                        });
                    }
                },//end success
