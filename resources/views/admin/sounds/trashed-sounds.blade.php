...
@push('js')
    ...
    <script>
        window.data_url = "{!! route('admin.get.trashed.sounds') !!}";
        window.columns = [
            {data: "id"},
            {data: "sound_image"},
            {data: "name_ar"},
    ...
    <script type="text/javascript">
        ...
        ///////////////////////////////////////////////////
        /// force Destroy Sound
        $(document).on('click', '.force_delete_sound_btn', function (e) {
            e.preventDefault();
            var id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    // Delete User
                    $.ajax({
                        url: '{!! route('admin.force.destroy.sound') !!}',
                        data: {id, id},
                        type: 'post',
                        dataType: 'json',
        ...
        ////////////////////////////////////////////////////
        // restore  Sound
        $(document).on('click', '.restore_sound_btn', function (e) {
            e.preventDefault();
            var id = $(this).data('id');

            $.ajax({
                url: "{{route('admin.restore.sound')}}",
                data: {id, id},
                type: 'post',
                dataType: 'JSON',
                beforeSend: function () {
                ...
                success: function (data) {
                    ...
                    if (data.status == true) {
                        Swal.fire({
                            ...
                            icon: "success",
                            allowOutsideClick: false,
                            customClass: {confirmButton: 'restore_sound_button'}
                        });
                        $('.restore_sound_button').click(function () {
                            updateDataTable();
                        });
                    }
                },//end success
