...
@push('js')
    ...
    <script>
        window.data_url = "{!! route('admin.get.trashed.programs') !!}";
        window.columns = [
            {data: "id"},
            {data: "icon"},
            {data: "name_ar"},
    ...
    <script type="text/javascript">
        ...
        ///////////////////////////////////////////////////
        /// force Destroy  Program
        $(document).on('click', '.force_delete_program_btn', function (e) {
            e.preventDefault();
            var id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    // Delete User
                    $.ajax({
                        url: '{!! route('admin.force.destroy.program') !!}',
                        data: {id, id},
                        type: 'post',
                        dataType: 'json',
        ...
        ////////////////////////////////////////////////////
        // restore  Programs
        $(document).on('click', '.restore_program_btn', function (e) {
            e.preventDefault();
            var id = $(this).data('id');

            $.ajax({
                url: "{{route('admin.restore.program')}}",
                data: {id, id},
                type: 'post',
                dataType: 'JSON',
                beforeSend: function () {
                ...
                success: function (data) {
                    ...
                    if (data.status == true) {
                        Swal.fire({
                            ...
                            icon: "success",
                            allowOutsideClick: false,
                            customClass: {confirmButton: 'restore_program_button'}
                        });
                        $('.restore_program_button').click(function () {
                            updateDataTable();
                        });
                    }
                },//end success
