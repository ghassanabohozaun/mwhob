...
@push('js')
    ...
    <script>
        window.data_url = "{!! route('admin.get.trashed.contests') !!}";
        window.columns = [
            {data: "id"},
            {data: "contest_image"},
            {data: "name_ar"},
    ...
    <script type="text/javascript">
        ...
        ///////////////////////////////////////////////////
        /// force Destroy Contest
        $(document).on('click', '.force_delete_contest_btn', function (e) {
            e.preventDefault();
            var id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    // Delete User
                    $.ajax({
                        url: '{!! route('admin.force.destroy.contest') !!}',
                        data: {id, id},
                        type: 'post',
                        dataType: 'json',
        ...
        ////////////////////////////////////////////////////
        // restore  Contest
        $(document).on('click', '.restore_contest_btn', function (e) {
            e.preventDefault();
            var id = $(this).data('id');

            $.ajax({
                url: "{{route('admin.restore.contest')}}",
                data: {id, id},
                type: 'post',
                dataType: 'JSON',
                beforeSend: function () {
                ...
                success: function (data) {
                    ...
                    if (data.status == true) {
                        Swal.fire({
                            ...
                            icon: "success",
                            allowOutsideClick: false,
                            customClass: {confirmButton: 'restore_contest_button'}
                        });
                        $('.restore_contest_button').click(function () {
                            updateDataTable();
                        });
                    }
                },//end success
